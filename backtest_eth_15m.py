...
def backtest(df: pd.DataFrame):
    ...
    for _, row in df.iterrows():
        ...
        # ===== 没持仓：今天还没开单 → 强制开（直到 equity <= 0） =====
        if (not in_pos) and (not opened_today) and equity > 0:
...
def summarize(df: pd.DataFrame, equity, trades):
    ...
    lines.append("========== 回测结果（每天强制一单 + 动态仓位 + 日内止损/追踪） ==========")
    lines.append(f"总交易数: {n}  | 覆盖率(有开单的天/总天): {trade_days}/{unique_days} ({coverage:.2f}%)")
