...
def backtest(df):
    ...
    for i in range(1, len(df)):
        ...
        dt, h, l, c = row["dt"], row["high"], row["low"], row["close"]
        ...
        if in_pos:
            ...
            if trail_mode > 0:
                stop_price = max(stop_price, high_since_entry * (1 - (TRAIL_1_PCT if trail_mode == 1 else TRAIL_2_PCT)))
            ...
            if exit_price:
                ...
                pnl = (exit_price - entry_price) * size - notional * FEE_RATE - abs(exit_price * size) * FEE_RATE
                equity += pnl
                trades.append({
                    "entry_time": entry_time, "exit_time": dt,
                    "entry_price": entry_price, "exit_price": exit_price,
                    "pnl_net": pnl, "equity_after": equity
                })
                in_pos = False
                entry_price = None
        ...
        if not in_pos and equity >= MARGIN_PER_TRADE:
            if is_uptrend(row) and (
                row["close"] > row["ema144"] or
                (prev_row["close"] <= prev_row["ema144"] and row["close"] > row["ema144"])
            ):
                in_pos = True
                entry_price = c
                entry_time = dt
...
def summarize(df, equity, trades):
    ...
    avg_loss = sum(t["pnl_net"] for t in trades if t["pnl_net"] < 0) / (n - wins) if (n - wins) else 0
